...
class GMW_FL_Admin
{
    ...
    public static function form_settings_xprofile_fields($gmw_forms, $formID, $section, $option)
    {
        ...
        if (function_exists('bp_has_profile')) {
            ...
            if (bp_has_profile('hide_empty_fields=0')) {
                ...
                while (bp_profile_groups()) {
                    ...
                    while (bp_profile_fields()) {
                        ...
                        if ((bp_get_the_profile_field_type() == 'datebox' || bp_get_the_profile_field_type() == 'birthdate')) {
                            ...
                            ?>
                            <input type="checkbox"
                                   name="<?php echo 'gmw_forms[' . $formID . '][' . $section . '][profile_fields][]'; ?>"
                                   value="<?php echo $field_id; ?>" <?php if (isset($gmw_forms[$formID][$section]['profile_fields']) && in_array($field_id, $gmw_forms[$formID][$section]['profile_fields'])) echo ' checked=checked'; ?>/>
                ...
                <label><strong
                        style="margin:5px 0px;float:left;width:100%"><?php _e('Choose the "Age Range" Field', 'GMW'); ?></strong></label>
                <br/>
                <select name="<?php echo 'gmw_forms[' . $formID . '][' . $section . '][profile_fields_date]'; ?>">
                    ...
                    foreach ($dateboxes as $datebox) {
                        ...
                        ?>
                        <option
                            value="<?php echo $datebox; ?>" <?php echo $selected; ?> ><?php echo $field->name; ?></option>
                    <?php } ?>
                </select>
                <?php
            }
    ...
    public function show_avatar($gmw_forms, $formID, $section, $option)
    {
        ...
        <div>
            <p>
                <input type="checkbox"
                       name="<?php echo 'gmw_forms[' . $_GET['formID'] . '][' . $section . '][avatar][use]'; ?>"
                       value="1" <?php echo (isset($gmw_forms[$formID][$section]['avatar']['use'])) ? "checked=checked" : ""; ?> />
            ...
            <p>
                ...
                &nbsp;<input type="text" size="5"
                             name="<?php echo 'gmw_forms[' . $_GET['formID'] . '][' . $section . '][avatar][width]'; ?>"
                             value="<?php echo (isset($gmw_forms[$formID][$section]['avatar']['width']) && !empty($gmw_forms[$formID][$section]['avatar']['width'])) ? $gmw_forms[$formID][$section]['avatar']['width'] : '200px'; ?>"/>
            </p>

            <p>
                <?php _e('Height', 'GMW'); ?>:
                &nbsp;<input type="text" size="5"
                             name="<?php echo 'gmw_forms[' . $_GET['formID'] . '][' . $section . '][avatar][height]'; ?>"
                             value="<?php echo (isset($gmw_forms[$formID][$section]['avatar']['height']) && !empty($gmw_forms[$formID][$section]['avatar']['height'])) ? $gmw_forms[$formID][$section]['avatar']['height'] : '200px'; ?>"/>
            </p>
        </div>
        <?php
    }
