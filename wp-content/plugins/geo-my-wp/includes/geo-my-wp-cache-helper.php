...
class GEO_my_WP_Cache_Helper
{
    ...
    */
    ...
    {
        self::get_transient_version('gmw_get_' . sanitize_text_field($taxonomy), true);
    }
    ...
    public static function clear_expired_transients()
    {
        ...
        if (!wp_using_ext_object_cache() && !defined('WP_SETUP_CONFIG') && !defined('WP_INSTALLING')) {
            ...
            $rows = $wpdb->query($wpdb->prepare($sql, $wpdb->esc_like('_transient_gmw_') . '%', $wpdb->esc_like('_transient_timeout_gmw_') . '%', time()));
        }
    }
}

GEO_my_WP_Cache_Helper::init();