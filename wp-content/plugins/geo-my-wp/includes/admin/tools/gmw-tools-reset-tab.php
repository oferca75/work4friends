...
function gmw_output_reset_tab()
{
    ...
    <div id="gmw-reset_gmw-tab-content" class="gmw-tools-tab-content">
        ...
        <div class="postbox">
            ...
            <div class="inside">
                <p>
                    <?php _e("Use this form to remove any data created by GEO my WP; Settings, forms, license keys and locations.", "GMW"); ?>
                    <br/>
                    <em style="color:red;font-weight:bold;font-size:14px;">
                        <?php _e("Very Important! This action cannot be undone so please be very careful when using it. Once you remove the data you wont be able to restore it unless you have a backup.", "GMW"); ?>
                        <br/>
                        <?php _e("By removing the post types locations database table you will lose all the post types locations created on this site.", "GMW"); ?>
                        <br/>
                        <?php _e("This includes locations create using GEO my WP, GEO Job Manager and Resume Manager Geolocation plugins.", "GMW"); ?>
                        <br/>
                ...
                <form method="post" action="<?php echo admin_url('admin.php?page=gmw-tools&tab=reset_gmw'); ?>">
                    <p>
                        <span><input type="checkbox" class="cb-clear-item" name="clear_item[]"
                                     value="settings"/><?php _e('Settings', 'GMW'); ?></span>
                        <span><input type="checkbox" class="cb-clear-item" name="clear_item[]"
                                     value="forms"/><?php _e('Forms', 'GMW'); ?></span>
                        <span><input type="checkbox" class="cb-clear-item" name="clear_item[]"
                                     value="licenses"/><?php _e('License Keys', 'GMW'); ?></span>
                        ...
                        if (count($ptTable) != 0) {
                            ?>
                            <span>
							<input type="checkbox" class="cb-clear-item" name="clear_item[]" value="pt_table"
                                   onchange="jQuery('#pt-remove-cb').toggle();"/>
                                <?php printf(__('Posts Locator Database Table ( %splaces_locator )', 'GMW'), $wpdb->prefix); ?>
                                <br/><em id="pt-remove-cb"
                                         style="display:block;display:none;color:red;margin-left:20px">this will
                                    permanently remove all the post types locations created on this site</em>
						</span>
                            <?php
                        }
                        ...
                        if (count($flTable) != 0) {
                            ?>
                            <span>
							<input type="checkbox" class="cb-clear-item" name="clear_item[]" value="fl_table"
                                   onchange="jQuery('#fl-remove-cb').toggle();"/>
                                <?php _e('Members Locator Database Table ( wppl_friends_locator ) - members location', 'GMW'); ?>
                                <br/><em id="fl-remove-cb"
                                         style="display:block;display:none;color:red;margin-left:20px">this will
                                    permanently remove all members locations created on this site</em>
						</span>
                        <?php } ?>
                        <span><input type="checkbox" class="cb-clear-item" name="clear_item[]"
                                     value="gmw_uninstall"/><?php _e('Completely Uninstall and deactivate GEO my WP', 'GMW'); ?></span>
                    </p>

                    <p>
                        <input type="hidden" name="gmw_action" value="clear_data"/>
                        <?php wp_nonce_field('gmw_clear_data_nonce', 'gmw_clear_data_nonce'); ?>
                        <input type="submit" class="button-secondary" value="<?php _e('Clear Data', 'GMW'); ?>"
                               id="gmw-clear-data-button"/>
                    </p>
                    <script>
