        ...
        public function output()
        {
            ...
            <div class="wrap">
                ...
                <div class="gmw-addons-page-top-area">
                    <div></span><h3
                            style="display:inline-block;margin:2px 0px"><?php _e("Add-ons usage", "GMW"); ?></h3> - <a
                            href="#" id="addons-info-toggle"
                            onclick="jQuery('#addons-info-wrapper').slideToggle();"><?php _e("Show info", "GMW"); ?></a>
                    </div>
                ...
                <form method="post" action="" style="text-align:center">
                    ...
                    <div
                        class="gmw-addons-page-top-area <?php echo (!empty($updater)) ? 'updater-disabled' : 'updater-enabled'; ?>">

                        <p class="description"
                           style="margin-bottom: 10px;border-bottom: 1px solid #e5e5e5;padding-bottom: 10px;">
                        ...
                        <?php if (!empty($updater)) { ?>
                            <div>
                                <p class="description"
                                   style="color:red"><?php _e('The add-ons updater is disabled and will not check for new updates.', 'GMW'); ?></p>
                                <input type="submit" name="gmw_enable_updater" class="button-primary"
                                       value="<?php _e('Enable Updater', 'GMW'); ?>"/>
                                <input type="hidden" name="gmw_updater_action" value="enabled"/>
                            </div>
                        <?php } else { ?>
                            <div>
                                <p class="description"
                                   style="color:green"><?php _e('The add-ons updater is enabled and will check for updates when needed. You will be notified when new updates are avalible.', 'GMW'); ?></p>
                                <input type="submit" name="gmw_disable_updater" class="button-secondary"
                                       value="<?php _e('Disable Updater', 'GMW'); ?>"/>
                    ...
                    <?php foreach ($addons_data as $addon) : ?>
                        ...
                        <li class="gmw-single-addon-wrapper <?php echo $addon['name']; ?> first">
                            ...
                            <?php if (!empty($addon['core']) && empty($addon['activated'])) { ?>
                                <div class="gmw-ribbon-wrapper">
                                    <div class="gmw-ribbon free"><?php _e('Free Add-on', 'GMW'); ?></div>
                                </div>
                            <?php } ?>
                            ...
                            <?php if (empty($addon['installed']) && !empty($addon['new_addon'])) { ?>
                                <div class="gmw-ribbon-wrapper">
                                    <div class="gmw-ribbon blue"><?php _e('New Add-on', 'GMW'); ?></div>
                                </div>
                            <?php } ?>
                                    ...
                                    <div class="gmw-addon-desc-inner">
                                        ...
                                        <?php } elseif ($addon['update_avaliable'] == 2) { ?>
                                            <div class="gmw-addon-update-available-wrapper">
                                                <em>New version
                                                    of <?php esc_attr_e($addon['title']); ?> <?php esc_attr_e($addon['current_version']); ?>
                                                    is available for
                                                    update. <?php echo GMW_Admin::update_addon_link($plugins[$addon['basename']]['Name']); ?></em>.
                                            </div>
                                        <?php } ?>
                                    </div>
                                </div>
                            ...
                            <?php if (!isset($this->addons) || !isset($this->addons[$addon['name']]) || $this->addons[$addon['name']] == 'inactive') { ?>
                                            ...
                                            <td class="plugin-update" colspan="3">
                                                ...
                                                <div class="gmw-license-key-fields-wrapper">
                                                    ...
                                                    <form method="post" action="">
                                                        ...
                                                        <div
                                                            class="gmw-license-wrapper gmw-license-invalid-wrapper addon-deactivated">
                                ...
                                <div
                                    class="gmw-addon-license-wrapper gmw-license-invalid gmw-addon-deactivate <?php if (empty($addon['installed'])) echo 'not-installed'; ?>">
                                    ...
                                    <?php if (!empty($addon['installed'])) { ?>
                                        ...
                                        <form method="post" action="">
                                            ...
                                            <input type="submit"
                                                   class="button-secondary button-primary gmw-addon-activation-btn"
                                                   value="<?php _e('Activate Add-on', 'GMW'); ?>"/>
                                            <input type="hidden" name="gmw_addon_basename"
                                                   value="<?php echo plugin_basename(esc_attr($addon['file'])); ?>"/>
                                            <input type="hidden" name="gmw_action" value="activate_addon"/>
                                            <input type="hidden" name="gmw_addon_activated"
                                                   value="<?php echo esc_attr($addon['name']); ?>"/>
                                            <?php wp_nonce_field($addon['name'] . '_activate_addon_nonce', $addon['name'] . '_activate_addon_nonce'); ?>
                                        </form>

                                    <?php } else { ?>
                                        <a href="<?php echo esc_url($addon['premalink']); ?>"
                                           class="button-secondary button-primary gmw-addon-activation-btn"
                                           target="_blank" title="Get add-on"><?php _e('Get Add-on', 'GMW'); ?></a>
                                    <?php } ?>
                                ...
                                <?php if (isset($addon['name'])) { ?>
                                    ...
                                    <div class="gmw-addon-license-wrapper gmw-addon-activate">
                                        ...
                                        <form method="post" action="">
                                            ...
                                            <input type="hidden" name="gmw_addon_deactivated"
                                                   value="<?php echo esc_attr($addon['name']); ?>"/>
