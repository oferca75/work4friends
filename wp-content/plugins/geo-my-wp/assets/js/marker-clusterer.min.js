...
ClusterIcon.prototype.onAdd = function () {
    ...
    google.maps.event.addDomListener(this.div_, "click", function (d) {
        b = !1;
        if (!c) {
            var e, g, h = a.cluster_.getMarkerClusterer();
            google.maps.event.trigger(h, "click", a.cluster_);
            google.maps.event.trigger(h, "clusterclick", a.cluster_);
            var l = h.getMap().getZoom(), k = a.cluster_.map_.getMapTypeId();
            if ("roadmap" == k || "satellite" == k)var f = 19, k = 3E-5; else f = 15, k = 7E-4;
            if (l >= f && 1 < a.cluster_.markers_.length) {
                d = a.cluster_.markers_;
                l = 360 / d.length;
                a.cluster_.map_.getZoom();
                for (f = 0; f < d.length; f++) {
                    var m = d[f].getPosition(), n = m.lat() + -k * Math.cos(+l * f / 180 * Math.PI), m = m.lng() + -k * Math.sin(+l * f / 180 * Math.PI), n = new google.maps.LatLng(n, m);
                    d[f].setPosition(n);
                    d[f].setMap(a.cluster_.map_)
                }
                a.hide()
            } else h.getZoomOnClick() && (g = h.getMaxZoom(), e = a.cluster_.getBounds(), h.getMap().fitBounds(e), setTimeout(function () {
                h.getMap().fitBounds(e);
                null !== g && h.getMap().getZoom() > g && h.getMap().setZoom(g + 1)
...
ClusterIcon.prototype.show = function () {
    if (this.div_) {
        var a = "", b = this.backgroundPosition_.split(" "), c = parseInt(b[0].replace(/^\s+|\s+$/g, ""), 10), b = parseInt(b[1].replace(/^\s+|\s+$/g, ""), 10), a = this.getPosFromLatLng_(this.center_);
        this.div_.style.cssText = this.createCss(a);
        a = "<img src='" + this.url_ + "' style='position: absolute; top: " + b + "px; left: " + c + "px; ";
        this.cluster_.getMarkerClusterer().enableRetinaIcons_ || (a += "clip: rect(" + -1 * b + "px, " + (-1 * c + this.width_) + "px, " + (-1 * b + this.height_) + "px, " + -1 * c + "px);");
        ...
        this.div_.title = "undefined" === typeof this.sums_.title || "" === this.sums_.title ?
            this.cluster_.getMarkerClusterer().getTitle() : this.sums_.title;
...
ClusterIcon.prototype.useStyle = function (a) {
    ...
    this.width_ = a.width;
    this.anchorText_ = a.anchorText || [0, 0];
...
Cluster.prototype.addMarker = function (a) {
    ...
    this.center_ ? this.averageCenter_ && (c = this.markers_.length + 1, b = (this.center_.lat() * (c - 1) + a.getPosition().lat()) / c, c = (this.center_.lng() * (c - 1) + a.getPosition().lng()) / c, this.center_ = new google.maps.LatLng(b, c), this.calculateBounds_()) : (this.center_ = a.getPosition(), this.calculateBounds_());
    a.isAdded = !0;
    this.markers_.push(a);
    b = this.markers_.length;
    c = this.markerClusterer_.getMaxZoom();
    if (null !== c && this.map_.getZoom() >
        c)a.getMap() !== this.map_ && a.setMap(this.map_); else if (b < this.minClusterSize_)a.getMap() !== this.map_ && a.setMap(this.map_); else if (b === this.minClusterSize_)for (a = 0; a < b; a++)this.markers_[a].setMap(null); else a.setMap(null);
    this.updateIcon_();
    return !0
};
...
Cluster.prototype.updateIcon_ = function () {
    ...
    null !== b && this.map_.getZoom() > b ? this.clusterIcon_.hide() : a < this.minClusterSize_ ? this.clusterIcon_.hide() : (a = this.markerClusterer_.getStyles().length, a = this.markerClusterer_.getCalculator()(this.markers_, a), this.clusterIcon_.setCenter(this.center_), this.clusterIcon_.useStyle(a), this.clusterIcon_.show())
};
...
function MarkerClusterer(a, b, c) {
    ...
    this.batchSizeIE_ =
        c.batchSizeIE || MarkerClusterer.BATCH_SIZE_IE;
...
MarkerClusterer.prototype.setupStyles_ = function () {
    var a, b;
    if (!(0 < this.styles_.length))for (a = 0; a < this.imageSizes_.length; a++)b = this.imageSizes_[a], this.styles_.push({
...
MarkerClusterer.prototype.addMarkers = function (a, b) {
    for (var c in a)a.hasOwnProperty(c) && this.pushMarkerTo_(a[c]);
    b || this.redraw_()
};
...
MarkerClusterer.prototype.resetViewport_ = function (a) {
    ...
    this.clusters_ = [];
    for (b = 0; b < this.markers_.length; b++)c = this.markers_[b], c.isAdded = !1, a && c.setMap(null)
};
MarkerClusterer.prototype.distanceBetweenPoints_ = function (a, b) {
    var c = (b.lat() - a.lat()) * Math.PI / 180, d = (b.lng() - a.lng()) * Math.PI / 180, c = Math.sin(c / 2) * Math.sin(c / 2) + Math.cos(a.lat() * Math.PI / 180) * Math.cos(b.lat() * Math.PI / 180) * Math.sin(d / 2) * Math.sin(d / 2);
...
MarkerClusterer.prototype.addToClosestCluster_ = function (a) {
    var b, c, d, e = 4E4, g = null;
    for (b = 0; b < this.clusters_.length; b++)if (d = this.clusters_[b], c = d.getCenter())c = this.distanceBetweenPoints_(c, a.getPosition()), c < e && (e = c, g = d);
...
MarkerClusterer.prototype.createClusters_ = function (a) {
    ...
    if (this.ready_) {
        ...
        c = 3 < this.getMap().getZoom() ? new google.maps.LatLngBounds(this.getMap().getBounds().getSouthWest(), this.getMap().getBounds().getNorthEast()) : new google.maps.LatLngBounds(new google.maps.LatLng(85.02070771743472, -178.48388434375), new google.maps.LatLng(-85.08136444384544,
            178.00048865625));
        c = this.getExtendedBounds(c);
        for (var e = Math.min(a + this.batchSize_, this.markers_.length); a < e; a++)b = this.markers_[a], !b.isAdded && this.isMarkerInBounds_(b, c) && (!this.ignoreHidden_ || this.ignoreHidden_ && b.getVisible()) && this.addToClosestCluster_(b);
        e < this.markers_.length ? this.timerRefStatic = setTimeout(function () {
            d.createClusters_(e)
...
MarkerClusterer.CALCULATOR = function (a, b) {
    for (var c = 0, d = a.length.toString(), e = d; 0 !== e;)e = parseInt(e / 10, 10), c++;
    c = Math.min(c, b);
    return {text: d, index: c, title: ""}
};
